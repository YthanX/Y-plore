...
volatile uint16_t ili9341_init_stage = 0;
...
void ili9341_Init(void)
{
  ...
  ili9341_init_stage = 2;
  LCD_Delay(10);
  LCD_IO_WriteCmd8(ILI9341_SWRESET);
  LCD_Delay(120);

#if ILI9341_DEBUG_MIN_INIT == 1
  /* Minimal sequence for step-by-step debug: if this still stays white, issue is likely pin/bus wiring. */
  ili9341_init_stage = 3;
  LCD_IO_WriteCmd8MultipleData8(ILI9341_PIXFMT, (uint8_t *)"\x55", 1);
  ili9341_init_stage = 4;
  LCD_IO_WriteCmd8(ILI9341_MADCTL);
  LCD_IO_WriteData8(ILI9341_MAD_DATA_RIGHT_THEN_DOWN);
  ili9341_init_stage = 5;
  LCD_IO_WriteCmd8(ILI9341_SLPOUT);
  LCD_Delay(120);
  ili9341_init_stage = 6;
  LCD_IO_WriteCmd8(ILI9341_DISPON);
  LCD_Delay(20);
  ili9341_init_stage = 7;
#else
  LCD_Delay(10);
